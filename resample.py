...
# Default paths
path_in = r'F:\DATA\hecktor2022\imagesTr'
path_label_in = r'F:\DATA\hecktor2022\labelsTr'
path_out = r'F:\DATA\HECKTOR\DATA\train_and_val\resampled_2=0'
path_bb = r'F:\DATA\hecktor2022\bb_box_training.csv'
...
def main(input_folder, input_label_folder, output_folder, bounding_boxes_file, cores, resampling):
    ...
    def resample_one_patient(p):
        ...
        if not os.path.exists(output_folder / p):
            os.makedirs(output_folder / p, exist_ok=True)
        sitk.WriteImage(ct, str(
            (output_folder / p / (p + "_ct.nii.gz")).resolve()))
        sitk.WriteImage(pt, str(
            (output_folder / p / (p + "_pt.nii.gz")).resolve()))
        sitk.WriteImage(gtvt,
                        str((output_folder / p / (p + "_ct_gtvt.nii.gz")).resolve()))
