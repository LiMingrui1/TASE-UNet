...
class FastSmoothSENormDeepUNet_supervision_skip_no_drop(nn.Module):
    ...
    def __init__(self, in_channels, n_cls, n_filters, reduction=2, return_logits=False):
        ...
        self.ca0 = CBAM(n_filters)
        # self.block_1_3_left = BasicConv3d(n_filters, n_filters, kernel_size=1, stride=1)
        ...
        # self.block_2_4_left = BasicConv3d(2 * n_filters, 2 * n_filters, kernel_size=1, stride=1)
        ...
        # self.block_3_4_left = BasicConv3d(4 * n_filters, 4 * n_filters, kernel_size=1, stride=1)
        ...
        # self.block_4_4_left = BasicConv3d(8 * n_filters, 8 * n_filters, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        ds33 = self.ca3(ds3)
        x = self.ca4(x)
        x = self.block_4_2_right(self.block_4_1_right(torch.cat([self.upconv_4(x), ds33], 1)))
        sv4 = self.vision_4(x)

        x = self.block_3_2_right(self.block_3_1_right(torch.cat([self.upconv_3(x), ds22], 1)))
        sv3 = self.vision_3(x)
